...
openai.api_key = "TU_OPENAI_API_KEY"  # reemplaza con tu clave real
CHUNK_SIZE = 1500  # tamaño de fragmento reducido para evitar timeouts
...
def traducir_fragmento(fragmento):
    ...
    for intento in range(3):
        try:
            ...
            print("⚠️  Timeout, reintentando en 5 segundos…")
            time.sleep(5)
        except Exception as e:
            print("❌ Error irrecuperable en OpenAI:", e)
            break
    return ""
...
for idx, (title, start, end) in enumerate(chapters, 1):
    ...
    for j, chunk in enumerate(chunk_text(narr), 1):
        ...
        parte = traducir_fragmento(chunk)
        trad += parte + "\n\n"
        print(" OK" if parte else " SKIP")
